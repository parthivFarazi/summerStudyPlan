...
| **Graphs (BFS/DFS)** | **2/5** | [graphs](graphs.md) — *Day 33. **implicit (grid, neighbours computed) vs explicit (objects, neighbours stored)** — same traversal, different neighbour source. #200 (permanent mark ⇒ O(m·n), not 4^L), #133 (dict = visited AND mapping; register the shell before recursing). **O(V + E)**: undirected stores 2E entries*
