...
| M-036 | 2026-07-28 | impl | **#133 — the wrong object's `neighbors` list, three sessions running.** D33/D34: `Node(node.val, node.neighbors)` handed the clone the ORIGINALS (measured: 5 reachable nodes instead of 4). **D35: `for neighbor in adict[node].neighbors` walked the CLONE's empty list — the loop body never ran, returning one isolated node. ⚠️ He had his notes OPEN and still mis-picked** | **Two objects share the attribute name `neighbors` and he cannot reliably tell them apart.** Not recall — the notes were in front of him. **Fix is mechanical, not attentional: name the clone.** `clone = Node(node.val, [])` … `for neighbor in node.neighbors: clone.neighbors.append(dfs(neighbor))`. Kin to M-021/B-5. **🔴 Day 38 — a completely new face, and it cost ~1 hour on `#91`.** He would not accept `ways(0) = 1`, arguing *"if it's empty shouldn't it be 0 because there are 0 letters to it"*. **He counted the CONTENTS (letters) when asked for the CONTAINERS (decodings):** `decodings("") = { [] }` — ONE element, which happens to be empty — versus `decodings("0") = { }` — ZERO elements. **`{ [] }` is not `{ }`.** Four prose explanations failed; only a step-through visualisation showing the actual decoding SET at each dp cell landed it. He wrote `ways("") = 0` three times, twice straight after being handed `1`, and on `"10"` reached the right total from **two cancelling errors**. **No graph, no naming slip — the same inability to hold a container distinct from its contents.** | **4** | Day 33, 34, 35, **38** | **⛔ B-5 REOPENED — clean streak reset to 0 (Day 38)** |
...
| M-042 | 2026-08-03 | strategy | **Priced a doubly-recursive function as `O(n)`.** #70: *"it computes n, then n−1, then n−2… so closer to O(n)"* — that describes a **chain**; each call makes TWO calls, so it is a branching tree. Measured: n=40 ⇒ **204,668,309 calls** | **A recursion that calls itself twice is `O(2ⁿ)` unless something stops the repetition.** B-10's other face: he priced one call, not the number of calls. **🔴 Day 38, one day later, identical:** wrote the un-memoized `#91` recursion and stated `O(n)` — *"one pass costs `O(1)`, there are `n` passes"*. **Measured: 3,524,577 calls at n=30; growth ×11.1 per +5 chars (`O(φⁿ)`); 1.15×10²¹ calls ≈ 7.3 million YEARS at the n=100 ceiling.** 🔑 **The sharper root cause, new on Day 38: he priced the algorithm IN HIS HEAD (a loop over n), not the code ON HIS SCREEN (a branching recursion).** `O(1)` per pass was even correct — for the loop he never wrote. **⛔ Drill: before any complexity, "is this a LOOP or a RECURSION?" If recursion, ask "how many CALLS", never "how many passes".** | **2** | Day 37, **Day 38** | **active — B-10 family, escalating** |
| M-043 | 2026-08-04 | strategy | **States a formula and does not test it on a single concrete case.** Day 38, three in one hour: `2ⁿ` substrings (died to listing `"abc"` — six, not eight) · `n!` for the count (died to `n=4` — ten, not twenty-four) · `right - left + 1` for the slice length (died to substituting `left=-1, right=3` — five, not three). **On the third, the disproving trace with both numbers was ON SCREEN and he replied *"that is the right answer"* without substituting** | **Formulas are produced by pattern-match against a recently-drilled problem and then believed.** `2ⁿ` is the `#78` Subsets number; `n!` fits `n=3` by coincidence (`3+2+1 = 6 = 3!`). **His reasoning is strong; his VERIFICATION is the gap.** Fix: **one concrete case, substituted, before believing your own formula** — five seconds, and it killed all three. Corollary: **never validate a formula on `n=3`** — too small to distinguish sum from product | 1 | **Day 38** | **active** |
| M-044 | 2026-08-04 | strategy | **Cannot instrument his own code.** `#91` crashed on 3 of 4 examples; asked to add one `print` at the top of the function and read the trace, he said *"I can't see, I don't know how to do it. I use the NeetCode testing always."* Coach had to run it for him to surface `s[-1:1] = ''` | **He has never debugged outside a hosted test runner.** `GOALS.md` records that interviewers probe *"what does this line do — remove it, what breaks?"*; **in an interview nobody hands you a harness.** Print-and-read-a-trace is the single highest-leverage debugging skill and he does not have it. Fix: **every crash from now on, HE adds the print and reads the trace before the coach runs anything** | 1 | **Day 38** | **active** |
...
## Day 38 addendum — an hour lost to a base case, and a complexity error repeated one day later

### 🟢 What he produced unaided
**`#5` was derived end to end by him**, from *"I have no clue how to start."* Substring-vs-subsequence (*"there is always b in the middle and it could never be formed"*), the triangular recurrence `T(n) = T(n−1) + n` **written one day after learning what a recurrence is**, both palindrome parities from peeling `abcba` and `abba` by hand, and expand-around-centre in his own words: *"expand it outwards like a two-pointer, expand it until the letters each pointer is at isn't the same."* **First draft correct — 11,373 cases, zero failures** — including reconstructing `s[left+1:right]` off by one in **both** directions, correctly, the day after `#200` failed on exactly that.

**And on `#91` he produced the counter-example BEFORE the formula, unprompted** — *"this pattern doesn't really work if it is 246"* — ten minutes after M-043 was named to him. **That is the fastest a correction has stuck.**

### 🔴 The hour — and it was not DP
`#91` took **1:42:00**, and the recurrence was written inside the first twenty minutes. **The remaining hour was `ways(0) = 1`** — see M-036 above. **The blocker was arithmetic-of-counting, not dynamic programming.** Budget for "what kind of object is this" on 2-D DP too.

### 🔴 The complexity error is now a pattern, not an incident
M-042 fired on consecutive sessions with the identical shape. **The generalisation is worth more than either instance: he prices his INTENT, not his CODE.** On Day 37 the intent was "walk down the chain" and the code branched; on Day 38 the intent was "loop over n" and the code recursed. **Both times the number he said was right about a program he had not written.**

### 🟢 B-10's first genuine movement
Two complexities volunteered **unasked** (`2n−1` centres; the memoized `#91` bound, with **both** space sources named — dictionary *and* call stack, his weak side). And he **self-corrected the fixed-width-slice trap**: asked whether `s[i-2:i]` makes a pass `O(n)` he said yes, then reversed himself into the full drill sentence — *"it is always O(2) so essentially it is O(1) so a single pass costs O(1), and there are n passes."* **Day 37 volunteered zero.**

### The slice rule, now stated once and for all
**A slice costs `O(k)` where `k` is its WIDTH, and `len()` is always `O(1)`.** `s[left+1:right]` on `#5` has width up to `n` (measured 1.3–1.5× overhead — real, but not the bound). `s[i-2:i]` on `#91` has width 2 forever. **Same syntax, different cost — read the indices, not the operator.** This is the exact line that carried Mock #1's 5.67 s TLE.

---

## Day 38 tail addendum (Aug 5) — what a clean session looks like, and the two things still wrong

**Block 1: 7 of 7 in ~22 minutes**, after 2-of-8 and 0-of-7. Four full solves verified against **independent** oracles — union-find, path enumerator, subset enumerator — 48,000+ inputs, zero failures.

### 🟢 Two repairs landed without being asked
**`#200`:** both Day-37 single-character bugs gone — `grid[row][col] = "0"` is an assignment, bounds read `0 <= nr < len(grid)`. **`#198`:** he guarded `[5]` unprompted; the same code without that guard still throws `IndexError`. **Neither needed telling twice.**

### 🟢 Rule 14, quantified
He self-caught two `#746` bugs by running the code. **The range off-by-one returns `dp[n-1]` instead of `dp[n]` and is wrong on 1,415 of 2,000 random arrays — 71%.** Found in 3:39 because he ran it.

### 🔴 M-006 / B-10 — the amortized gap *(new facet, not a new mistake)*
`#739`: *"each iteration requires O(1) work."* **False** — the inner `while` can pop repeatedly, so one pass can cost `O(n)`. The `O(n)` bound is **amortized**: each index is pushed once, popped at most once. **When a loop contains a loop, per-pass pricing is the wrong instrument.** Counts as a precision gap on an otherwise 8-for-8 session.

### 🔴 M-029 / B-8
One narration slip: *"create a stack called `answer`"*, then used `stack[-1]` and `answer[popped]` as two distinct objects. **The stack and the result array are separate; name them separately.**

### 🔴 M-034 kin — right by unreachability *(watch)*
`#91`'s space-optimised version initialises `dp[0]` **conditionally** when `dp[0]` is unconditionally 1. Verified harmless — identical output on 1,119,110 strings — because both readers of `dp[0]` are guarded by conditions that already force `s[0]` valid. **Correct today, wrong after any refactor that removes either guard.** Not logged as a failure; logged as a habit to name: *does this value have to be right, or does it merely happen never to be read?*

### 📌 Coach errors this session
1. **Wrote annotated `.py` files into his working folder** (→ `COACHING.md` rule 21). macOS case-insensitivity meant `block2DecodeWays.py` silently overwrote his `block2decodeWays.py`; **his comments in it are unrecoverable.**
2. **Ran the Socratic method on a base case for ~1 hour** (→ rule 20). The same student learned the same class of thing in ~2 minutes the next day when shown a table of objects instead.
