...
| M-021 | 2026-07-13 | impl | **Container vs. contents** — index vs. `"#"` instead of `s[j]` (#271, Day 21); `.val` vs the `.left`/`.right` pointers (#226, Day 21); #271 AGAIN identical index-vs-char (Day 22); `left`/`right` HEIGHTS to count NODES (Day 22). **Day 23 CLEAN ✅ · Day 24 CLEAN ✅** | Reaching for the handle instead of the thing it points at: `i` is a position, `s[i]` is the char; `.left` is a NODE, `.val` is a number. **And: a variable being in scope does not make it relevant** | 4 | Day 21 (×2), Day 22 (×2) | **B-5 CLEARED Day 24 ✅ (2 clean)** |
...
*Pre-empt these at the start of each session. (M-004 → B-1 cleared Day 16; M-003 → B-2 cleared Day 18; M-001 → B-3 cleared Day 21 [**watch reopened Day 24**]; **M-011 → B-4 cleared Day 24; M-021 → B-5 cleared Day 24**.)*

**Active blockers: B-6 (M-012, search direction) · B-7 (M-020, `self.` confusion)** — see `BLOCKERS.md`. **B-3 (M-001, return) on watch.**
