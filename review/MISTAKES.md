...
| M-004 | 2026-06-19 | impl | Wrong variable/container — `nums.add` vs `seen.add`; compared `s` vs `clean` (#125) | Losing track of which variable/container holds what | 2 | Day 1, Day 9 | active |
...
| M-004 | impl | Wrong variable/container reference (`s` vs `clean`, etc.) | 2 |

*Pre-empt these five at the start of each session — they're the ones about to become blockers. M-004 is the one to watch: one more and it's a drilled blocker.*
