...
### B-7 · M-020 — the `self.` / attribute-ownership rule  🟡 *(DRILL-NOW — 1 clean session Day 29)*
Cleared Day 26, fired Day 27 (#211 `self.root`/`node.isEnd`) + Day 28 (#146 `remove(lru)`). **Day 29: HELD — #211 ownership all correct first-draft, #146's `self.remove` correct in the sibling branch (the one miss was a confirmed paste-drop). The morning micro-drill also confirmed his *understanding* is sound** — the real gap was the nested-function (bare) vs class-method (`self.`) distinction, and he had it. So this is an **execution** slip, and the fix is the final read-through, not the rule.
**Drill (one more session): before submit, point at every attribute/method and ask "whose thing is this?"** — `self.x=`/method of this class → `self.`; a field → `object.field`; param/local/nested-`def` → bare. **1 of 2 clean → clears on one more clean session.**
